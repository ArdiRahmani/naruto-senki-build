...
class Sasuke : public Hero
{
	void setID(const string &name, Role role, Group group) override
	{
		...
		match_char_exp(HeroEnum::Sasuke, setAIHandler(Sasuke::perform),
	                	HeroEnum::ImmortalSasuke, setAIHandler(Sasuke::perform_ImmortalSasuke),
					   HeroEnum::SasukeRinnegan, setAIHandler(Sasuke::perform_SasukeRinnegan));
	}

	void changeAction() override
	{
		match_char_exp(HeroEnum::Sasuke, changeAction_Sasuke(),
			          	HeroEnum::ImmortalSasuke, changeAction_ImmortalSasuke(),
					   HeroEnum::SasukeRinnegan, changeAction_SasukeRinnegan());
	}

	void resumeAction(float dt) override
	{
		match_char_exp(HeroEnum::Sasuke, resumeAction_Sasuke(dt),
					   HeroEnum::ImmortalSasuke, resumeAction_ImmortalSasuke(dt),
					   HeroEnum::SasukeRinnegan, resumeAction_SasukeRinnegan(dt));
	}

	void setActionResume() override
	{
		match_char_exp(HeroEnum::Sasuke, setActionResume_Sasuke(),
					   HeroEnum::ImmortalSasuke, setActionResume_ImmortalSasuke(),
					   HeroEnum::SasukeRinnegan, setActionResume_SasukeRinnegan());
	}
	...
	inline void changeAction_ImmortalSasuke()
	{
		if (_skillChangeBuffValue == 17)
		{
			_isTaunt = true;
	...
	inline void resumeAction_ImmortalSasuke(float dt)
	{
		if (_isTaunt)
		{
			_isTaunt = false;
	...
	inline void setActionResume_ImmortalSasuke()
	{
		if (_isTaunt)
		{
			unschedule(schedule_selector(Sasuke::resumeAction_ImmortalSasuke));
			_isTaunt = false;
			setHurtAction(createAnimation(hurtArray, 10, false, true));
			_skillChangeBuffValue = 0;
		}
...
void perform_SasukeRinnegan()
		...
		if (_mainTarget && _mainTarget->isNotFlog())
		{
			...
			if (isFreeState())
			{
				...
				else if (abs(sp.x) < 32 || _isArmored)
				{
					...
					else if ((abs(sp.x) > 16 || abs(sp.y) > 32) && !_isCanSkill1 && !_isCanSkill2)
					{
						moveDirection = sp.getNormalized();
						walk(moveDirection);
						return;
	...
	inline void changeAction_SasukeRinnegan()
	{
		if (_skillChangeBuffValue == 17)
		{
			_isTaunt = true;
	...
	inline void resumeAction_SasukeRinnegan(float dt)
	{
		if (_isTaunt)
		{
			_isTaunt = false;
	...
	inline void setActionResume_SasukeRinnegan()
	{
		if (_isTaunt)
		{
			unschedule(schedule_selector(Sasuke::resumeAction_SasukeRinnegan));
			_isTaunt = false;
			setHurtAction(createAnimation(hurtArray, 10, false, true));
			_skillChangeBuffValue = 0;
		}
