...
class Naruto : public Hero
{
	...
	inline Hero *createClone_SageNaruto(int cloneTime)
	{
		...
		clone->setSkill1Action(clone->createAnimation(clone->skillSPC5Array, 10, false, true));
	...
	inline Hero *createClone_RikudoNaruto(int cloneTime)
{
    ...
    if (cloneTime == 0)
    {
        clone = createCloneHero<RikudoNarutoClone>(getName());
    }
    // 🔥 SUMMON KURAMA
    else if (cloneTime == 12)
    {
        clone = createSummonHero<Kurama>(SummonEnum::Kurama);
    }
    ...
    // 🔥 Fix animasi (khusus clone Naruto aja)
    if (cloneTime == 0)
    {
        clone->setSkill1Action(
            clone->createAnimation(clone->skillSPC5Array, 10, false, true)
        );
    }
